...
version: 1.0.0
generated_utc: 2026-04-15T11:45:27Z
---
...
| --- | --- | --- | --- |
| Household/profile name | Yes | Free text, short | Display and account grouping |
| Number of family members | Yes | Integer | Planning context and future scaling |
| School\-going kids | Yes | Yes / No | Tiffin relevance and weekday logic |
| Meal types to plan | Yes | Breakfast, lunch, dinner, tiffin, snack | Shows active planning contexts |
| Diet preference | Yes | Veg, egg, non\-veg, mixed | Catalog filtering and suggestion eligibility |
| Cuisine preference | Optional | South Indian, North Indian, Mixed, Other | Biases starter catalog and future suggestions |

### Functional behavior

- The app shall allow setup with only required household\-level fields.

- The app shall not force creation of individual profiles in MVP.

- The user shall be able to revise household settings later from Settings/Profile.
...
Bottom navigation should be used in MVP with three primary tabs: Home, Add Meal, and History. Settings/Profile can be surfaced via a top\-right entry point.
...
| Optional note field | Should support short comments and context. | Keep low prominence. |
| Save action | Must be fast and always available if minimum required data is present. | Do not over\-validate. |
...
- Edit household\-level setup fields.

- View and manage diet type and meal contexts.

- Optionally manage future features such as reminders or favorites when introduced.

- Provide privacy, support, and data controls when those are implemented.
...
| --- | --- | --- | --- |
| FR\-001 | The system shall support creation of a single household profile during onboarding. | Must | A new user can complete setup and reach Home without creating individual member profiles. |
| FR\-002 | The system shall capture family size, school\-going kids flag, meal types to plan, and diet preference during onboarding. | Must | These fields are stored and used in recommendation filtering. |
| FR\-003 | The system shall allow household settings to be edited after onboarding. | Should | Changes made in Settings are reflected in future recommendations. |
...
| --- | --- | --- |
| Household | household\_id, family\_size, has\_school\_kids, diet\_type, active\_meal\_types, cuisine\_preference | Stores household\-level planning context |
| MealEntry | meal\_id, household\_id, meal\_name, meal\_category, meal\_type, image\_uri, note, logged\_at, source | Stores what was cooked and when |
...
| RecommendationEvent | event\_id, household\_id, meal\_context, candidate\_ids, shown\_ids, selected\_id, timestamp | Supports analytics and future model training |
...
| --- | --- | --- | --- |
| /household | Create / update | Create or edit household profile | Can be local\-first with sync later |
| /meals | Create / list | Save meals and retrieve history | Supports optional image upload path |
| /meals/classify | Post | Return meal/category suggestions from image and note | Non\-blocking helper service |
| /feedback | Create | Store soft feedback against a meal | Lightweight write path |
| /recommendations | Get | Return ranked suggestions for a meal context | Central ranking service or local engine |
| /analytics/events | Post | Record exposure and interaction events | Can batch for efficiency |

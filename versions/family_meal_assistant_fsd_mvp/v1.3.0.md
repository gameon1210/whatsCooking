...
version: 1.3.0
generated_utc: 2026-04-18T08:15:00Z
---
...
- The AI meal-naming provider, model, and secret key shall be editable in Settings so the user can switch between supported providers without changing the app build.
...
- Configurable AI meal naming setup with provider, model, and encrypted secret key management.
...
- The app shall use the provider and model selected in Settings for AI meal naming, with the user allowed to change both later.
...
| --- | --- | --- |
| Broad meal understanding | Configurable multimodal service chosen in Settings \(ChatGPT, Claude, or Gemini in the current implementation\) with a user\-editable model string | Likely meal name or category, confidence, and possible meal type |
...
- The AI Setup page shall let the user edit provider, model, and secret key, and those values shall be applied to subsequent image classification requests.
...
| Filters | Must support meal type and household member filters. | Optional date range later. |
...
- Allow the user to choose the AI provider for meal naming from the supported provider list.

- Allow the user to edit the provider model string for meal naming.

- Require the user to create and paste their own provider\-specific secret key for meal naming.

- Store the secret key only in encrypted local storage on device.
...
| FR\-010 | The system shall support filtering history by meal type and household member. | Must | Selecting a filter updates the visible list correctly. |
...
| FR\-012 | The system shall show between two and three ranked recommendations on Home. | Must | The Home screen never shows zero suggestions unless there is a system failure state. |
...
| FR\-026 | The system shall allow the user to choose the active AI provider and model for meal naming from Settings. | Must | Changing provider/model affects later image classification requests without requiring a new build. |
| FR\-027 | The system shall require the user to provide their own provider\-specific secret key for AI meal naming. | Must | No provider secret is hardcoded or bundled in the application. |
| FR\-028 | The system shall store the secret key only in encrypted on\-device storage. | Must | The key is read and written exclusively through encrypted settings storage. |
| FR\-029 | The system shall validate member name, birth year, meal name, member selection, model input, and secret key input before save actions. | Must | Invalid values show inline guidance and block the corresponding save action. |
...
| Settings | ai\_provider, ai\_model, ai\_api\_key \(encrypted\), exploration\_ratio, api\_key\_banner\_dismissed, onboarding\_complete | Stores local app and AI configuration |
...
| --- | --- | --- | --- |
| SettingsRepository | Read / write | Save AI provider, model, encrypted secret key, and local app settings | Local only in MVP |
| MealRepository | Create / list | Save meals and retrieve history | Local Room storage |
| ImageClassifier | Classify | Return meal naming suggestion from image | Uses the user\-selected provider, model, and secret key |
...
# 14. Historical Gap Analysis Archive

Revision date: March 28, 2026  \|  Analyst: Claude Code  \|  Codebase: commit 3dbf239

This section is a historical archive of an earlier implementation review. It is retained for context only and is not the authoritative statement of the current codebase. For the current implementation, Sections 2 through 9 of this document are the authoritative requirements and behavior reference.
